...
def based_on_routes(area:ClimbingArea):
    return area.routes()
def sort_by_number_of_routes(areas:list):
    return sorted(areas,key=based_on_routes)

def based_on_hardest(area:ClimbingArea):
    return area.hardest_route().grade

def sort_by_most_difficult(areas:list):
    return sorted(areas,key=based_on_hardest,reverse=True)
